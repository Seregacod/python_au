import unittest
from main import Point, Triangle, TaskSolver
...
class TestTaskSolverMethods(unittest.TestCase):
    def test_parse_data(self):
        task_solver = TaskSolver('in01.txt', 'out.txt')
        task_solver.read_data()
        self.assertEqual(task_solver.parse_data(), [[0, 3, 0, 4, 0, 0.07, ],
                                                    [0, 0, 0, 3, 3, 0]])

        task_solver = TaskSolver('in02.txt', 'out.txt')
        task_solver.read_data()
    ...
    def test_find_largest_triangle(self):
        ...
        task_solver = TaskSolver('in03.txt', 'out.txt')
        task_solver.read_data()
        task_solver.parse_data()
